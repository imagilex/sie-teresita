...
if($comentario != "" && $indicador != "" && $id_indicador_nivel != "" && $id_usuario != "")
{
	...
	$usuario=$usr["usuario"];
	$niv=mysqli_fetch_array(consulta_directa($Con, "select nivel from indicador_dir_nivel where id_indicador_nivel='$id_indicador_nivel'"));
	$nivel=$niv["nivel"];
	$posicion=mysqli_fetch_array(consulta_directa($Con, "select count(*) as n from indicador_dir_comentario where indicador='$indicador' and nivel='$nivel' and anio = '$anio' and mes='$mes'"));
	$pos=intval($posicion["n"])+1;
	if($fecha=="")
		consulta_directa($Con, "insert into indicador_dir_comentario (anio, mes, indicador, pos, usuario, comentario, nivel, fecha) values ('$anio', '$mes', '$indicador', '$pos', '$usuario', '$comentario', '$nivel', curdate())");
	else
		consulta_directa($Con, "insert into indicador_dir_comentario (anio, mes, indicador, pos, usuario, comentario, nivel, fecha) values ('$anio', '$mes', '$indicador', '$pos', '$usuario', '$comentario', '$nivel', '$fecha')");
}
...
if($indicador != "" && $id_indicador_nivel != "")
{
	$niv=mysqli_fetch_array(consulta_directa($Con, "select nivel from indicador_dir_nivel where id_indicador_nivel='$id_indicador_nivel'"));
	$nivel=$niv["nivel"];
	if($usuarios=consulta_directa($Con, "select nombre, clave from persona where clave in (select distinct(persona) from usuario where clave in (select distinct(usuario) from indicador_dir_comentario where indicador='$indicador' and nivel='$nivel' and anio = '$anio' and mes='$mes'))"))
	{
		while($usr=mysqli_fetch_array($usuarios))
		{
			?>
				<fieldset>
					...
					<ul type="circle">
						<?php
						if($comentarios=consulta_directa($Con, "select comentario, fecha, usuario from indicador_dir_comentario where indicador='$indicador' and nivel='$nivel' and anio = '$anio' and mes='$mes' and usuario in (select clave from usuario where persona = '".$usr["clave"]."') order by fecha"))
						{
