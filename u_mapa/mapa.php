...

{
...

	{
...

		{
			...
			<table border="0" align="left">
				...
				<tr>
					...
					<td align="left">
						...
						<select name="mapa" id="mapa" onchange="javascript: location.href='<?php echo $link_page; ?>id_mapa='+this.value;"><option value=""><?php echo $this->get_nombre(); ?></option>						
			...
			</td><td width="1114" align="center"></td>
...

			{
				...
				<center>
				  ...
				  <iframe src="_resultado_consulta_vs_pantalla_for_list.php?pantalla=9&consulta=<?php echo $this->get_contenido(); ?>&encabezado=falso&noCache="+math.random()+"&sess_usua=<?php echo $sid_usuario; ?>" width="95%" height="400" marginheight="0" marginwidth="0" frameborder="0" name="Reporte" id="Reporte"></iframe>
...

			{
				...
				<table border="0" width="95%" cellpadding="0" cellspacing="0"><tr><td class="contenedor" align="left">
					...
					<?php
...

						{
...

							{
...

								{
									...
									<td class="<?php echo (($id_documento==$prod["id_documento"])?("celda_actual"):("celda_normal")); ?>" style="font-size:small;" onclick="javascript: location.href='<?php echo $link_page; ?>id_mapa=<?php echo $this->get_id(); ?>&docto=<?php echo $prod["id_documento"]; ?>';"><?php echo $prod["nombre_documento"];?></td>
						...
						<form style="padding:0px; margin:0px;" action="usr_rh68.php" method="post" name="data_del">
							...
							<input type="submit" name="btnDelDocto" value="Borrar" style="width:75px; height:25px;" />
						...
						<td style="font-size:small;">
...

							{
								...
								case 3:	//tipo archivo
									...
									<table border="0" align="center"><tr><td heigh="200"><center><!--<input name="button" type="button" style="width:100px; height:25px; border-width:1px; border-style:solid;" onclick="javascript: window.open('map_doc/<?php echo $prod["contenido"]; ?>');" value="Archivo" />--><iframe height="350" width="900px;" marginwidth="0" marginheight="1" frameborder="0" src="map_doc/<?php echo $prod["contenido"]; ?>" align="middle"></iframe></center></td></tr></table>
			...
			else if($this->get_tipo()=="5") //Tipo Organizacion - Area

			{

				$area = $this->get_contenido();				

				$query="select lista from area_lista where area = '$area' and tipo='D'";

				$listas=$db->consulta($query);

				while($lista=mysql_fetch_array($listas))

				{

					$nombre=mysql_fetch_array($db->consulta("select nombre from lista where lista = '".$lista["lista"]."'"));

					?>

					<strong><?php echo $nombre["nombre"]; ?></strong>

					<ul style="padding: 0px 0px; margin: 0px auto; text-align: center; list-style: none;">

						<?php

						$query="select persona from lista_persona where lista = '".$lista["lista"]."' order by secuencia";
...

						{
							...
							$query="select clave, nombre, apaterno, amaterno, imagen from persona where clave = '".$persona["persona"]."'";

							$datos=mysql_fetch_array($db->consulta($query));

							?>

							<li style="display: inline; "><img src="img_personas/<?php echo $datos["imagen"];?>" align="<?php echo $datos["nombre"]." ".$datos["apaterno"]." ".$datos["amaterno"]; ?>" title="<?php echo $datos["nombre"]." ".$datos["apaterno"]." ".$datos["amaterno"]; ?>" /></li>
			...
			else if($this->get_tipo()=="6") //Tipo Personal

			{

				$persona = $this->get_contenido();

				echo "<h1>Persona: $persona</h1>";
