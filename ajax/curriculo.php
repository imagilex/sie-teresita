...
$dbasicos=@mysql_fetch_array($db->consulta("select fecha_nacimiento,estado_civil, sexo, rfc from persona where clave='$dato'"));

$edo_civil=mysql_fetch_array(mysql_query("select descripcion from codigos_generales where campo='Estado_civil' and valor='".$dbasicos["estado_civil"]."'"));
...
$direc=@mysql_fetch_array($db->consulta($query));
$query="select * from codigo_postal where cp='".$direc["codigo_postal"]."'";
$direc2=@mysql_fetch_array($db->consulta($query));
?>
<table align="center" width="90%" style="text-transform:capitalize;">
	<tr>
		<td width="60%" valign="top" align="left">
			<table border="0">
				<tr><td align="left"><?php echo htmlentities($edad." a�os"); ?></td></tr>
				...
				{
					while($est=mysql_fetch_array($ests))
					{
						$campo="";
						...
						$carr=mysql_fetch_array($db->consulta($query));
						if($est["p_nivel"]!="")
						{
		...
		<td width="40%" valign="top" align="center">
			<table border="0">
				...
				{
					while($medio=mysql_fetch_array($medios))
					{
