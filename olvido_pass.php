...
{
	$usuario=@mysql_fetch_array(mysql_query("select clave as usuario, password as contrasenia from usuario where persona in (select distinct(clave) from persona where email='$correo')"));
	if($usuario["usuario"]!="" && $usuario["contrasenia"]!="")
	{
		$email_cuerpo="";
		$Arch=mysql_fetch_array(mysql_query("select valor  from seccion where id_seccion='mail' and elemento='olvido_pass'"));
