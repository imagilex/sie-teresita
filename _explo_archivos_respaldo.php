...
$activacion="";
	...
	@mysql_free_result($regs);
		...
		//$idea='a<input style="visibility:hidden;"  type="checkbox" name="archivo[]" value="'.$direct.'" onclick="'.$activacion.'" /><input type="hidden" name="" value="'.$reg["clave"].'" />';
			...
			{ 
				?><img src="Imagenes/archivo.JPG" align="absmiddle" border="0" /><input type="hidden" name="tipo<?php echo $x; ?>" id="tipo<?php echo $x; ?>" value="archivo" /><input type="hidden" name="valor<?php echo $x; ?>" id="valor<?php echo $x; ?>" value="<?php echo "$ruta/$direct"; ?>" /><?
			}
		}
		//echo $direct;
		if($Bloqueado_por=="")
