...
{	
	...
	$usuario=$usr["usuario"];
	if($fecha=="")
		mysql_query("insert into reporte_comentario (id_reporte, usuario, comentario, fecha, fecha_reporte) values ('$id_reporte', '$usuario', '$comentario', curdate(), '$fecha_reporte')");
	else
		mysql_query("insert into reporte_comentario (id_reporte, usuario, comentario, fecha, fecha_reporte) values ('$id_reporte', '$usuario', '$comentario', '$fecha', '$fecha_reporte')");
}

if($id_reporte != "")
{
	if($usuarios=mysql_query("select nombre, clave from persona where clave in (select distinct(persona) from usuario where clave in (select distinct(usuario) from reporte_comentario where id_reporte='$id_reporte' and fecha_reporte='$fecha_reporte'))"))
	{
		while($usr=mysql_fetch_array($usuarios))
		{
			?>
				<fieldset>
					...
					<ul type="circle">
						<?php
						if($comentarios=mysql_query("select comentario, fecha, usuario from reporte_comentario where id_reporte='$id_reporte' and fecha_reporte='$fecha_reporte' and usuario in (select clave from usuario where persona = '".$usr["clave"]."') order by fecha"))
						{
