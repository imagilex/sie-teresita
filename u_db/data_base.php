...

{
...

	{
...

		{
			...
			$this->coneccion=mysql_connect($this->get_host(),$this->get_usuario(),$this->get_password());
...

	{
...

		{
			...
			$res=mysql_select_db($this->get_db(),$this->coneccion);
	...
	{	
		...
		if($this->get_conectado() && mysql_errno($this->coneccion)!="0")

			return mysql_errno($this->coneccion);
...

	{
...

		{
...

			{
				...
				<script language="javascript">window.alert("<?php echo mysql_error($this->coneccion); ?>");</script>

				<?php

			}

			return mysql_error($this->coneccion);
...

	{
...

		{
			...
			$cuantos=mysql_fetch_array($this->consulta("select count(*) as n from $bd.$tabla"));
